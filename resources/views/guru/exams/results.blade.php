                    ...
                    <div class="mb-6 border-b border-gray-200">
                        <nav class="-mb-px flex space-x-6" aria-label="Tabs">
                            <a href="{{ request()->fullUrlWithQuery(['tab' => 'attempts']) }}"
                                ...
                                <span class="inline-flex items-center justify-center px-2 py-0.5 ml-1 text-xs font-bold leading-none rounded-full {{ $activeTab === 'attempts' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-600' }}">
                                    {{ $statistics['total_attempts'] }}
                                </span>
                            </a>
                            <a href="{{ request()->fullUrlWithQuery(['tab' => 'unattempted']) }}"
                                class="flex items-center gap-2 pb-4 px-1 border-b-2 text-sm font-semibold transition-all duration-200 {{ $activeTab === 'unattempted' ? 'border-purple-600 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                                <i class="fas fa-user-slash"></i>
                                {{ __('Belum Mengerjakan') }}
                                <span class="inline-flex items-center justify-center px-2 py-0.5 ml-1 text-xs font-bold leading-none rounded-full {{ $activeTab === 'unattempted' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-600' }}">
                                    {{ $unattemptedCount }}
                                </span>
                            </a>
                        </nav>
                        ...
                        <div>
                            <h3 class="text-lg font-bold text-gray-900">
                                @if ($activeTab === 'unattempted')
                                    <i class="fas fa-user-slash text-purple-600 mr-2"></i>Daftar Siswa Belum Mengerjakan Ujian
                                @else
                            ...
                            <p class="text-sm text-gray-500 mt-1">
                                @if ($activeTab === 'unattempted')
                                    Menampilkan {{ $unattemptedUsers->total() }} siswa
                                @else
                                    Menampilkan {{ $attempts->total() }} percobaan (status selesai)
                                @endif
                            </p>
                        </div>
                    ...
                    @if ($activeTab === 'unattempted')
                        ...
                        @else
                            <div class="flex flex-col items-center justify-center text-gray-500 py-12">
                                ...
                                <p class="text-sm font-semibold">Semua siswa sudah mengerjakan ujian ini.</p>
                            </div>
                        @endif
                    @else
                        <div class="overflow-x-auto border border-gray-200 rounded-lg">
