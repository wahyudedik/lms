<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route(auth()->user()->getRolePrefix() . '.information-cards.create') }}"
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <div class="flex flex-wrap gap-2 text-xs text-gray-500">
                                    <span><i class="fas fa-users mr-1"></i>{{ __('Target') }}:
                            ...
                            <div class="flex items-center gap-2 ml-4">
                                <form method="POST"
                                    action="{{ route(auth()->user()->getRolePrefix() . '.information-cards.toggle-status', $card) }}">
                                    @csrf
                                    <button type="submit"
                                ...
                                <a href="{{ route(auth()->user()->getRolePrefix() . '.information-cards.edit', $card) }}"
                                    class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg" title="{{ __('Edit') }}">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form method="POST"
                                    action="{{ route(auth()->user()->getRolePrefix() . '.information-cards.destroy', $card) }}"
