    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Material - :title', ['title' => $course->title]) }}
            </h2>
                ...
                <div class="p-6">
                    <form action="{{ route('admin.courses.materials.update', [$course, $material]) }}" method="POST"
                        ...
                        @if ($material->type === 'file' || $material->type === 'video')
                            @if ($material->file_path)
                                <div class="mb-4 p-4 bg-gray-50 rounded">
                                    <p class="text-sm text-gray-700">{{ __('File saat ini:') }}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label for="order"
                                    class="block text-sm font-medium text-gray-700 mb-2">{{ __('Urutan') }}</label>
                        ...
                        <div class="flex gap-4">
                            ...
                            <a href="{{ route('admin.courses.materials.index', $course) }}"
                                class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">{{ __('Batal') }}</a>
                        </div>
                    </form>
                </div>
            </div>
