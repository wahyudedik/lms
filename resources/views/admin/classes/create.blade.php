    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <i class="fas fa-school mr-2"></i>{{ __('Create New Class') }}
            </h2>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-md rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.classes.store') }}" method="POST" x-data="{ isGeneral: @js(old('is_general', false) ? true : false) }">
                        @csrf
                            ...
                            <div>
                                ...
                                <select name="education_level" id="education_level" :disabled="isGeneral"
                                    ...
                                    @foreach ($educationLevels as $value => $label)
                                        <option value="{{ $value }}"
                                            {{ old('education_level') === $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
