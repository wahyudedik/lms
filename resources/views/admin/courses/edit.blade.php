                    ...
                    <form action="{{ route('admin.courses.update', $course) }}" method="POST"
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-150 @error('description') border-red-500 @enderror">{{ old('description', $course->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            <label for="instructor_id" class="block text-sm font-semibold text-gray-700 mb-2">
                                <i class="fas fa-chalkboard-teacher text-gray-400 mr-1"></i>{{ __('Pengajar') }} <span class="text-red-500">*</span>
                            </label>
                            <select name="instructor_id" id="instructor_id" required
                                class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-150 @error('instructor_id') border-red-500 @enderror">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                <label for="status" class="block text-sm font-semibold text-gray-700 mb-2">
                                    <i class="fas fa-toggle-on text-gray-400 mr-1"></i>{{ __('Status') }} <span class="text-red-500">*</span>
                                </label>
                        ...
                        @if ($course->cover_image)
                            <div class="mb-6">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">
                                    <i class="fas fa-image text-gray-400 mr-1"></i>{{ __('Cover Saat Ini') }}
                                </label>
                        ...
                        <div class="mb-6">
                            ...
                            <p class="mt-1 text-sm text-gray-500">{{ __('Format: JPG, PNG, GIF. Maksimal 2MB. Biarkan kosong jika tidak ingin mengubah.') }}</p>
                            @error('cover_image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
