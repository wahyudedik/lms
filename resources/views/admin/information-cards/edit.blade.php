    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <i class="fas fa-edit mr-2"></i>{{ __('Edit Information Card') }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-md p-6">
                <form method="POST" action="{{ route('admin.information-cards.update', $informationCard) }}"
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title"
                                value="{{ old('title', $informationCard->title) }}" required
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Content') }} <span
                                    class="text-red-500">*</span></label>
                            <x-quill-editor name="content" :value="old('content', $informationCard->content)" :placeholder="__('Write the information card content here...')" />
                            @error('content')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="card_type" id="card_type"
                                    class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                                    <option value="info"
                                        {{ old('card_type', $informationCard->card_type) === 'info' ? 'selected' : '' }}>
                                        {{ __('Info (Blue)') }}</option>
                                    <option value="success"
                                        {{ old('card_type', $informationCard->card_type) === 'success' ? 'selected' : '' }}>
                                        {{ __('Success (Green)') }}</option>
                                    <option value="warning"
                                        {{ old('card_type', $informationCard->card_type) === 'warning' ? 'selected' : '' }}>
                                        {{ __('Warning (Yellow)') }}</option>
                                    <option value="danger"
                                        {{ old('card_type', $informationCard->card_type) === 'danger' ? 'selected' : '' }}>
                                        {{ __('Danger (Red)') }}</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Icon') }}</label>
                                <x-icon-picker name="icon" :value="old('icon', $informationCard->icon)" />
                            </div>
                        </div>

                        <!-- Attachment & Video -->
                        <x-information-card-media-fields :card="$informationCard" />
                                ...
                                @foreach ($roles as $role)
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="target_roles[]" value="{{ $role }}"
                                            {{ in_array($role, old('target_roles', $informationCard->target_roles)) ? 'checked' : '' }}
                        ...
                        <div x-data="{
                            targetMode: '{{ old('target_user_ids', $informationCard->target_user_ids) ? 'specific' : 'all' }}',
                            searchQuery: ''
                        }">
                            <label
                                    ...
                                    @foreach ($users as $user)
                                        <label
                                            ...
                                            <input type="checkbox" name="target_user_ids[]" value="{{ $user->id }}"
                                                {{ in_array($user->id, old('target_user_ids', $informationCard->target_user_ids ?? [])) ? 'checked' : '' }}
                                                class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                            ...
                            <div class="space-y-3">
                                <label class="flex items-center">
                                    <input type="radio" name="schedule_type" value="always"
                                        {{ old('schedule_type', $informationCard->schedule_type) === 'always' ? 'checked' : '' }}
                                ...
                                <label class="flex items-center">
                                    <input type="radio" name="schedule_type" value="date_range"
                                        {{ old('schedule_type', $informationCard->schedule_type) === 'date_range' ? 'checked' : '' }}
                                        class="text-blue-600 focus:ring-blue-500" onchange="toggleScheduleFields()">
                                    <span
                                ...
                                <label class="flex items-center">
                                    <input type="radio" name="schedule_type" value="daily"
                                        {{ old('schedule_type', $informationCard->schedule_type) === 'daily' ? 'checked' : '' }}
                                        class="text-blue-600 focus:ring-blue-500" onchange="toggleScheduleFields()">
                        ...
                        <div id="dateRangeFields" class="grid grid-cols-1 md:grid-cols-2 gap-4"
                            ...
                            <div>
                                ...
                                <input type="date" name="start_date" id="start_date"
                                    value="{{ old('start_date', $informationCard->start_date?->format('Y-m-d')) }}"
                                    class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                            </div>
                            <div>
                                <label for="end_date"
                                ...
                                <input type="date" name="end_date" id="end_date"
                                    value="{{ old('end_date', $informationCard->end_date?->format('Y-m-d')) }}"
                                    class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="sort_order" id="sort_order"
                                    value="{{ old('sort_order', $informationCard->sort_order) }}" min="0"
                                    class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <div class="flex items-center pt-6">
                                <label class="inline-flex items-center">
                                    ...
                                    <input type="checkbox" name="is_active" value="1"
                                        {{ old('is_active', $informationCard->is_active) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    <span
                    ...
                    <div class="mt-8 flex items-center justify-end gap-3">
                        ...
                        <button type="submit"
                            ...
                            <i class="fas fa-save mr-2"></i>{{ __('Update Card') }}
                        </button>
                    </div>
                </form>
            </div>
