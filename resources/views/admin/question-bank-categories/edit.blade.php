    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <i class="fas fa-folder-open mr-2"></i>Edit Kategori: {{ $questionBankCategory->name }}
            </h2>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-md rounded-lg">
                <div class="p-6">
                    <form method="POST"
                        action="{{ route('admin.question-bank-categories.update', $questionBankCategory) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-5">
                            ...
                            <input type="text" id="name" name="name"
                                value="{{ old('name', $questionBankCategory->name) }}" required
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="description" name="description" rows="3"
                                class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-150">{{ old('description', $questionBankCategory->description) }}</textarea>
                            ...
                            <select id="parent_id" name="parent_id"
                                ...
                                @foreach ($parents as $parent)
                                    <option value="{{ $parent->id }}"
                                        {{ old('parent_id', $questionBankCategory->parent_id) == $parent->id ? 'selected' : '' }}>
                                        {{ $parent->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-5">
                            <div>
                                ...
                                <div class="flex items-center gap-3">
                                    <input type="color" id="color" name="color"
                                        value="{{ old('color', $questionBankCategory->color) }}"
                            ...
                            <div>
                                ...
                                <input type="number" id="order" name="order"
                                    value="{{ old('order', $questionBankCategory->order) }}" min="0"
                                    class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-all duration-150">
                        ...
                        <div class="mb-6">
                            <label class="flex items-center gap-2">
                                <input type="checkbox" name="is_active" value="1"
                                    {{ old('is_active', $questionBankCategory->is_active) ? 'checked' : '' }}
