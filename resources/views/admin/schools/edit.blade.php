    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                ...
                {{ __('Edit School: :name', ['name' => $school->name]) }}
            </h2>
                ...
                <div class="p-6">
                    <form action="{{ route('admin.schools.update', $school) }}" method="POST"
                        ...
                        @if ($school->logo || $school->favicon)
                            <div class="mb-6">
                                <h3 class="text-lg font-bold text-gray-900 mb-4">
                                    <i class="fas fa-image mr-2"></i>{{ __('Current Branding') }}
                                </h3>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    @if ($school->logo)
                                        <div>
                                            <p class="text-sm font-medium text-gray-700 mb-2">{{ __('Logo:') }}</p>
                                    ...
                                    @if ($school->favicon)
                                        <div>
                                            <p class="text-sm font-medium text-gray-700 mb-2">{{ __('Favicon:') }}</p>
                        ...
                        <div class="mb-6">
                            <h3 class="text-lg font-bold text-gray-900 mb-4">
                                <i class="fas fa-palette mr-2"></i>{{ __('Update Branding') }}
                            </h3>

                            <!-- Logo -->
                            <div class="mb-4">
                                <label for="logo" class="block text-sm font-medium text-gray-700 mb-2">
                                    {{ __('New Logo (Optional)') }}
                                </label>
                                <input type="file" name="logo" id="logo"
                                    accept="image/jpeg,image/png,image/jpg,image/gif" class="w-full">
                                <p class="text-sm text-gray-500 mt-1">{{ __('Max 2MB. Leave empty to keep current') }}</p>
                                @error('logo')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                <label for="favicon" class="block text-sm font-medium text-gray-700 mb-2">
                                    {{ __('New Favicon (Optional)') }}
                                </label>
                                <input type="file" name="favicon" id="favicon" accept="image/x-icon,image/png"
                                    class="w-full">
                                <p class="text-sm text-gray-500 mt-1">{{ __('Max 512KB. Leave empty to keep current') }}</p>
                                @error('favicon')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-6 border-t">
                            ...
                            <button type="submit"
                                ...
                                <i class="fas fa-save mr-2"></i>{{ __('Update School') }}
                            </button>
                        </div>
                    </form>
                </div>
